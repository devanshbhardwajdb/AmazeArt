...
const handler = async (req, res) => {
    try {
        if (req.method === 'PUT') {
            ...
            const { username, reviewText, postId ,profilepic} = req.body;
            ...
            post.reviews.push({ username, reviewText, profilepic });

            // Save the updated post
            await post.save();

            res.status(200).json({ success: true, message: 'Review added successfully',post });
        } else {
